...
fut_data_extraction <- function(year_sel = NA, links_sel = NA, links_examined = 1:20, rate_sel = "80/20"){
  ...
  if (is.null(links_sel)) {
    links_sel <- get_all_match_urls(year = year_sel)
  }
  
  # Filter links_selected to only have links that haven't been used
  masc_links <- setdiff(links_sel$male, used_links)
  fem_links <- setdiff(links_sel$female, used_links)
  ...
  amount_male <- round(links_examined * as.numeric(relevance_rates["male"]))
  amount_female <- round(links_examined * as.numeric(relevance_rates["female"]))
  
  # Select links based on desired amount (with a stop if it's over the amount of links available)
  if (amount_male > length(masc_links)) {
    masc_links <- masc_links[1:length(masc_links)]
  } else {
    masc_links <- masc_links[1:amount_male]
  }
  
  if (amount_female > length(fem_links)) {
    fem_links <- fem_links[1:length(fem_links)]
  } else {
    fem_links <- fem_links[1:amount_female]
  }
  ...
  # Empty variables and data frames to save the data
  playersMatchLogs <- teamsMatchLogs <- c()
  fem_sh_logs <- masc_sh_logs <- fem_ply_keeper <- masc_ply_keeper <- fem_team_keeper <- masc_team_keeper <- data.frame()
  
  
  # Shooting Logs ------------
  print("Shooting Logs")
  
  # Extract shooting logs data and add column for sex
  ...
  # Extract player goalkeeping data and add column for sex
  ...
  # Extract team goalkeeping data and add column for sex
  ...
  # Player & Team Match Logs ------------
  for (stat in stat_types) {
    # Player stats
    print(paste(stat, "Player", sep = " - "))
  ...
  # Save data (skip empty data frames) ------------
  if (!is.null(sh_logs) | nrow(sh_logs) > 0) {
  ...
  if (!is.null(ply_keeper) | nrow(ply_keeper) > 0) {
  ...
  if (!is.null(team_keeper) | nrow(team_keeper) > 0) {
  ...
  if (!is.null(playersMatchLogs) | nrow(playersMatchLogs) > 0) {
  ...
  if (!is.null(teamsMatchLogs) | nrow(teamsMatchLogs) > 0) {
